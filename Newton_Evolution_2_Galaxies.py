...
INITIAL_WORLD_SIZE = [200, 200, 200]
SIMULATION_TIMES = 5
...
normal_pos = np.zeros((NO_OF_NORMAL_PARTICLES, 3))
dark_pos = np.zeros((NO_OF_DARK_PARTICLES, 3))

# normal_vel = np.zeros((NO_OF_NORMAL_PARTICLES, 3))
# normal_vel = np.zeros((NO_OF_NORMAL_PARTICLES, 3))

# Creating a galaxy:
centre = [50,50,50]
r = np.array(random.choices(range(50), weights=[np.exp(-0.1*i**2)/np.sqrt(np.pi/0.1) for i in range(50)], k=(NO_OF_NORMAL_PARTICLES//2)))
theta = np.random.rand(NO_OF_NORMAL_PARTICLES//2)
phi = np.random.rand(NO_OF_NORMAL_PARTICLES//2)
normal_pos[:(NO_OF_NORMAL_PARTICLES//2), 0] = r * np.cos(theta) * np.cos(phi) + 50
normal_pos[:(NO_OF_NORMAL_PARTICLES//2), 1] = r * np.cos(theta) * np.sin(phi) + 50
normal_pos[:(NO_OF_NORMAL_PARTICLES//2), 2] = r * np.sin(theta) * np.sin(phi) + 50

r = np.array(random.choices(range(50), weights=[np.exp(-0.1*i**2)/np.sqrt(np.pi/0.1) for i in range(50)], k=NO_OF_DARK_PARTICLES//2))
theta = np.random.rand(NO_OF_DARK_PARTICLES//2)
phi = np.random.rand(NO_OF_DARK_PARTICLES//2)
print(dark_pos[:(NO_OF_DARK_PARTICLES//2), 0].shape, theta.shape, phi.shape)
dark_pos[:(NO_OF_DARK_PARTICLES//2), 0] = r * np.cos(theta) * np.cos(phi) + 50
dark_pos[:(NO_OF_DARK_PARTICLES//2), 1] = r * np.cos(theta) * np.sin(phi) + 50
dark_pos[:(NO_OF_DARK_PARTICLES//2), 2] = r * np.sin(theta) * np.sin(phi) + 50

# Creating a second galaxy:
centre = [150,150,150]
r = np.array(random.choices(range(50), weights=[np.exp(-0.1*i**2)/np.sqrt(np.pi/0.1) for i in range(50)], k=(NO_OF_NORMAL_PARTICLES//2)))
theta = np.random.rand(NO_OF_NORMAL_PARTICLES//2)
phi = np.random.rand(NO_OF_NORMAL_PARTICLES//2)
normal_pos[(NO_OF_NORMAL_PARTICLES//2):, 0] = r * np.cos(theta) * np.cos(phi) + 150
normal_pos[(NO_OF_NORMAL_PARTICLES//2):, 1] = r * np.cos(theta) * np.sin(phi) + 150
normal_pos[(NO_OF_NORMAL_PARTICLES//2):, 2] = r * np.sin(theta) * np.sin(phi) + 150

r = np.array(random.choices(range(50), weights=[np.exp(-0.1*i**2)/np.sqrt(np.pi/0.1) for i in range(50)], k=NO_OF_DARK_PARTICLES//2))
theta = np.random.rand(NO_OF_DARK_PARTICLES//2)
phi = np.random.rand(NO_OF_DARK_PARTICLES//2)
dark_pos[(NO_OF_DARK_PARTICLES//2):, 0] = r * np.cos(theta) * np.cos(phi) + 150
dark_pos[(NO_OF_DARK_PARTICLES//2):, 1] = r * np.cos(theta) * np.sin(phi) + 150
dark_pos[(NO_OF_DARK_PARTICLES//2):, 2] = r * np.sin(theta) * np.sin(phi) + 150

print(np.max(normal_pos, axis=1))
